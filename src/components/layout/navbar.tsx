...
export default function Navbar() {
  ...
  // Pages with LIGHT hero backgrounds — need dark text when navbar is transparent
  const lightHeroPages = ['/about', '/gift-ideas','/order-success', '/login', '/register', '/account','/product']
  ...
  // Light hero pages get dark text; dark hero pages (home) get white text
      ...
      <nav style={{
        ...
        <div style={{
          ...
          <div className="sl-desktop-only"
            ...
            {/* Shop button — hover opens dropdown, click goes to /shop */}
            <div style={{ position: 'relative' }}>
              <button
                ...
                <svg width="11" height="11" viewBox="0 0 24 24" fill="none"
                  ...
                  style={{ transition: 'transform 0.3s', transform: shopOpen ? 'rotate(180deg)' : 'none' }}>
            ...
            {[
              ...
              <Link key={label} to={path} className="sl-nav-link"
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '1.4rem', flex: 1, justifyContent: 'flex-end' }}>
            ...
            <Link to="/cart" title="Cart"
              ...
              <span style={{
                ...
                background: cartBg, color: '#FFF', borderRadius: '50%',
                width: '15px', height: '15px', fontSize: '0.55rem', fontWeight: 700,
            ...
            <button
              ...
              style={{
                background: 'none', border: 'none', cursor: 'pointer',
                color: textColor, padding: '6px', lineHeight: 0,
                transition: 'color 0.4s',
              }}>
              {mobileOpen
        ...
        {mobileOpen && (
          <div style={{
            ...
            borderTop: '1px solid rgba(138,79,177,0.15)',
            padding: '1rem 2rem 2rem',
            maxHeight: '80vh', overflowY: 'auto',
            animation: 'sl-mobileIn 0.25s ease',
          }}>
            <Link to="/" onClick={() => setMobileOpen(false)} className="sl-mob-link">Home</Link>

            {/* Shop accordion */}
            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid rgba(138,79,177,0.12)' }}>
                <Link to="/shop" onClick={() => setMobileOpen(false)} className="sl-mob-link"
                  style={{ border: 'none', flex: 1 }}>Shop</Link>
                <button
                  ...
                  {shopCategories.map(col => (
                    <div key={col.heading} style={{ marginBottom: '0.75rem' }}>
                      <p style={{
                        ...
                        color: '#8A4FB1', margin: '0.8rem 0 0.4rem',
                      }}>{col.heading}</p>
                      {col.links.map(link => (
                        <Link key={link.to} to={link.to}
                          ...
                          style={{
                            ...
                            color: '#5B21B6', textDecoration: 'none',
                            padding: '0.25rem 0',
                            transition: 'color 0.2s',
                          }}
            ...
            {[
              ...
              <Link key={label} to={path}
                ...
                className="sl-mob-link">{label}</Link>
            ))}
          </div>
        )}
      </nav>

      {/* ── DROPDOWN — outside nav to prevent clipping ── */}
      {shopOpen && (
        <div
          onMouseEnter={handleMouseEnterShop}
          onMouseLeave={handleMouseLeaveShop}
          style={{
            ...
            borderTop: '2px solid #8A4FB1',
            borderBottom: '1px solid rgba(138,79,177,0.15)',
            boxShadow: '0 16px 48px rgba(59,16,88,0.10)',
            zIndex: 9998,
            padding: '2.5rem 4rem',
            display: 'grid',
            gridTemplateColumns: 'repeat(4, 1fr)',
            gap: '2.5rem',
            animation: 'sl-dropDown 0.2s ease',
          }}
        >
          {shopCategories.map(col => (
            <div key={col.heading}>
              <p style={{
                ...
                letterSpacing: '0.18em', textTransform: 'uppercase',
                color: '#8A4FB1', marginBottom: '0.9rem', marginTop: 0,
              }}>{col.heading}</p>
              {col.links.map(link => (
                <Link
                  key={link.to}
                  to={link.to}
                  className="sl-drop-link"
                  onClick={() => {
                    setShopOpen(false)
                    if (closeTimer.current) clearTimeout(closeTimer.current)
                  }}
